...
window.addEventListener('load', () => {
  // const inputsReq = document.getElementsByClassName('obrigatorio');
  // console.log(inputsReq);
  // console.log(inputsReq[0].value);

  // for (i=0; i<= inputsReq.length; i++) {
  //   console.log(inputsReq[i]);
  // }
...
function listagem() {
  ...
  for (i = 0; i < dados.length; i++) {
    ...
    for (j = 0; j < client.length; j++) {
      if (j >= 3 && j < 9) {
        if (client[j].length) {
          end = client[4] + ', nº ' + client[5] + '  ' + client[6] + '<br>';
          end += 'Bairro: ' + client[7] + ', Cidade: ' + client[8] + ' - ' + client[3];
        }
